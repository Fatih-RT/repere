    ...
    queryFn: async (): Promise<StatsData> => {
      ...
      const [logs, questions, pomos, sessions] = await Promise.all([
        pb.collection("review_logs").getFullList<ReviewLog>({
          filter: pb.filter("reviewed_at >= {:start}", { start: earliestStart.toISOString() }),
        ...
        pb.collection("pomodoro_sessions").getFullList<{ started_at: string; actual_seconds: number }>({
          filter: pb.filter("started_at >= {:start} && phase = \"focus\"", { start: appDayStart(new Date(now.getTime() - 13 * 86400000), tz, cutoff).toISOString() }),
          fields: "started_at,actual_seconds",
        }),
        pb.collection("review_sessions").getFullList<{ started_at: string; ended_at: string }>({
          filter: pb.filter("started_at >= {:start} && ended_at != \"\"", { start: appDayStart(new Date(now.getTime() - 13 * 86400000), tz, cutoff).toISOString() }),
      ...
      // --- mastery ---
      const masteredCount = questions.filter((q) => masteryPct(q.interval_days) >= 70).length;
      const masteredNewThisWeek = questions.filter((q) => masteryPct(q.interval_days) >= 70 && q.last_reviewed_at && dayOffsetOf(q.last_reviewed_at) < 7).length;
      const hardQuestions = questions.filter((q) => easeToDifficulty(q.ease_factor) === "Difficile");
      ...
      return {
        ...
        masteredCount, totalActive: questions.length, masteredNewThisWeek,
